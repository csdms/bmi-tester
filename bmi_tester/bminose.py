...
def run_tests(
    bmi_cls, infile=None, bmi_version=None, mode="fast", verbose=True, doctests=False
):
...
def main():
    ...
    parser.add_argument(
        "--infile", default="", help="Name of input file for init method."
    )
    parser.add_argument(
        "--bmi-version", default="1.1", help="BMI version to test against"
    )
    parser.add_argument(
        "-m",
        "--mode",
        ...
        dest="mode",
        default="fast",
        help='"fast", "full", or something that can be ' "passed to nosetests -A",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    try:
        ...
        print(
            "error loading {cls}: {msg}".format(
                cls=args.cls, msg=str(err), file=sys.stderr
            )
        )
        sys.exit(-1)
    else:

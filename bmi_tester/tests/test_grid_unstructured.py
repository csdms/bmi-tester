...
def test_grid_x():
    ...
    for gid in all_grids(bmi, gtype="unstructured"):
        ...
        _check_array_is_ndarray.description = "Test x for grid {gid} is ndarray".format(
            gid=gid
        )
        ...
        _check_array_length.description = "Test length of x for grid {gid}".format(
            gid=gid
        )
        ...
        _check_array_length.description = "Test x for grid {gid} is float".format(
            gid=gid
        )
